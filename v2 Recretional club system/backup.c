...
void reviewLeave() {
	...
	if (confirm == 'Y') {
		if (strcmp(tempStatus, "Approved") == 0) {
			...
			strcpy(tempLeave.leaveStatus, "Approved");
			...
			fprintf(writeApprovedLeaveRec, "%s|%d|%d-%d-%d|%d-%d-%d|%s|%s\n", tempLeave.ID, tempLeave.totalDay, tempLeave.sDay, tempLeave.sMonth, tempLeave.sYear, tempLeave.eDay, tempLeave.eMonth, tempLeave.eYear, tempLeave.reason, tempLeave.leaveStatus);
			...
			strcpy(rejected.reason, tempLeave.reason); strcpy(rejected.leaveStatus, tempStatus); strcpy(rejected.rejectReason, rejectReason);
			FILE* writeResponse;
...
void updateStaffLeaveStatus() {
	...
	for (int i = 0; i < totalLeaveRecord; i++) {
		...
		validateEndOfLeaveDate(&tempRec.eDay, &tempRec.eMonth, &tempRec.eYear);
		if (tempRec.eDay == sysDay && tempRec.eMonth == sysMonth && tempRec.eYear == sysYear) {
			fprintf(appendResponseRec, "%s|%d|%d-%d-%d|%d-%d-%d|%s|%s\n", approvedLeave[i].ID, approvedLeave[i].totalDay, approvedLeave[i].sDay, approvedLeave[i].sMonth, approvedLeave[i].sYear, approvedLeave[i].eDay, approvedLeave[i].eMonth, approvedLeave[i].eYear, approvedLeave[i].reason, approvedLeave[i].leaveStatus);
		}
		else {
			fprintf(writeApprovedRec, "%s|%d|%d-%d-%d|%d-%d-%d|%s|%s\n", approvedLeave[i].ID, approvedLeave[i].totalDay, approvedLeave[i].sDay, approvedLeave[i].sMonth, approvedLeave[i].sYear, approvedLeave[i].eDay, approvedLeave[i].eMonth, approvedLeave[i].eYear, approvedLeave[i].reason, approvedLeave[i].leaveStatus);
		}
